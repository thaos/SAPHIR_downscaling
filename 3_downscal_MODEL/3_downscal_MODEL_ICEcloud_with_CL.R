##Script MODEL = calcul du model qrf pour chaque couche d HR pour les nuages de glace et avec CloudSat
...
## FONCTION de DOWNSCALING sur l echantillon d apprentissage
 clustering_downscall<-function(area,year,month.num,layer)
 {
  ...
  filename.dir.out <- paste0(output,"model_ICE_cloud_avec_CloudSat/QRF_model_entire_timeseries/",year,"/")
  ...
  Cloud_type<-apply(match.CALIPSO.SAPHIR.CLOUDSAT, 1, function(r) any(r == 2))
  new_data_2<-match.CALIPSO.SAPHIR.CLOUDSAT[Cloud_type,] 
  ...
  saveRDS(qrf, file = paste0(filename.dir.out,"qrf_",area,"_",month.num,"_",year,"MODEL_",layer,".rds"))  # pour l echantillon traiter avec 50000 obs CALIPSO (voir ligne 55)
  saveRDS(qrf, file = paste0(filename.dir.out,"qrf_",area,"_",month.num,"_",year,"MODEL_",layer,"_entire_learn_sample.rds"))  # pour l echantillon traiter avec 50000 obs CALIPSO (voir ligne 56)
