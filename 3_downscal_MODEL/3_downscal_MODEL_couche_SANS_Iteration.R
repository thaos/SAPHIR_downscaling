...
#---------------------------------------------------------------------------------------------------
### Fonction QRF sans la partie iterative

###############################################################################################################################
####### QRF iterative pour CALIPSO et CLOUDSAT
QRF_CALIPSO_CLOUDSAT <- function( data,tol,nmaxiter,alg,verbose=FALSE){
  ...
  qrf$rsq_sss<-rsq  # pour avoir le rsq de la boucle. rsq pour chaque prediction!!!
  qrf$predict<-dep.fine 
  return(qrf)
}
...
clustering_downscall<-function(area,year,month.num,layer)
{
 ...
 ## --------------------------------------------------------------------------------------------------------------------------------
    ...
    qrf <- QRF_CALIPSO_CLOUDSAT(data=x,tol,nmaxiter,alg)   #default: ntree=500 and mtry=predictors/3)
